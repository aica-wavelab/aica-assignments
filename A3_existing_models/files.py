...
def load_midi_files(files: str | list[str], time_step: float | None= None, max_files: float | None=None, transpose_to_major: bool=False) -> list[m21.stream.Stream]:
    """
    ...
    if isinstance(files, str):
        files_ =  glob.glob(f'{files}/**/*.mid',recursive = True)
    else:
        files_ = files
    for file_name in files_:
        midi_stream = m21.converter.parse(file_name)
        if time_step is None or has_acceptable_duration(midi_stream, time_step):
            if transpose_to_major:
                midi_stream = transpose(midi_stream)
            if max_files is not None and max_files <= len(midi_streams):
                break
            else:
                midi_streams.append(midi_stream)
    return midi_streams

def transpose(stream: m21.stream.Stream) -> m21.stream.Stream:
